...
if($opc == 1)
{
		...
		if(isset($_POST["mensajecorreo"]))
		{
				...
				if(count($_POST['listaasociar']) > 0 && trim($listas) != "" && $listas != ",")
				{
					...
					if($num > 0)
					{
						...
						while($PSN->next_record())
						{
							...
							$xml = file_get_contents("https://sistemasmasivos.com/smsmasivo/api/sendsms/send.php?user=".urlencode("optica@iglesiaamoryrestauracion.org")."&password=-lOjoqY3L7&GSM=".$sms_celular."&SMSText=".$mensajecorreo);
							//
							$retorno = explode(",", $xml);
			...
			if($contadorOk > 0)
			{
				?><center><h1>.MENSAJES ENVIADO CON EXITO: <u><?=$contadorOk; ?></u>.</h1></center><?
			}
			if($contadorError > 0)
			{
			...
			*/
			...
			if($saldo_actual > 0)
			{
				...
				<thead>
					<tr> 
					<th colspan="4">.ENVIO DE MENSAJES.</th>
					</tr>
				</thead>
				<tbody>
					<tr> 
						<td><strong><label for="mensajecorreo">Mensaje</label>:</strong></td>
						<td><textarea name="mensajecorreo" id="mensajecorreo" cols="40" rows="6"><?=soloNumeros($_POST["mensajecorreo"]); ?></textarea><br /><i>Recuerde que debe tener máximo 160 caracteres. Caracteres disponibles: <span id="contador_mensajes">160</span></i></td>
					</tr>
					<tr> 
				...
				<br />
				<center><input type="submit" name="button" value="Enviar Mensaje" style="background-color:#FFFFFF;border-color:#0000FF;color:#006600;font-weight:bold"></center>
				</form>
				<?
			}
			else
