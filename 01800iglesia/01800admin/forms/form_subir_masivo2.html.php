...
<div id="form">
  <form action="<?= $servidor ?>/01800admin/?op=66&op2=4" method="post" name="subir_iglesias2" id="subir_iglesias2" enctype="multipart/form-data">
    ...
    <br>
    <label for="">Iglesia principal</label>&nbsp;&nbsp;
    <input type="text" name="iglesia_ppal" placeholder="Escriba el nombre de la iglesia principal..." id="iglesia_ppal" size="70" tabindex="1" />
    <br><br>
    	...
    	<thead>
      	<?php
					foreach($archivo['columnas_nombre'] as $columna) :
						echo "<th style='font-size: 85%'>".$columna."</th>";
					endforeach;
        ?>
      </thead>
    <?php endif; ?>
      ...
      if(isset($archivo['datos']) and !empty($archivo['datos'])) :
				foreach($archivo['datos'] as $fila) :
					...
					foreach($fila as $dato) :
						echo "<td style='font-size: 85%'>".$dato."</td>\t";
					endforeach;
					echo "</tr>\n";
				endforeach;
			endif;
