$(document).ready(function() {
	$( "#igl_id" ).autocomplete({ source: "index3.php?op=43&tipo=2" });	
	var v = $("#crear_lista").validate( {
		...
		messages: {
			...
			descripcion: {
				required: " <span id='msj_error_texto'>Escriba la descripción</span>",
				minlength: jQuery.format(" <span id='msj_error_texto'>Mínimo {0} caracteres necesarios!</span>")
			},
			email: {
...
//			igl_id: { required: " <span id='msj_error_texto'>Ingrese el nombre de la iglesia</span>" }
		...
		submitHandler: function() { jQuery("#crear_lista").submit(); }
	});
});