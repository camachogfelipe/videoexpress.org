...
$clave_acceso		= md5($_REQUEST['clave']);
$iglesia_congrega	= $_REQUEST['iglesia'];

//sacamos la fecha para determinar el codigo de usuario
$conecta = conecta_bd("videoexpress");
$sql = consulta_bd("actualizacion","num_afiliado, alquiler, afiliacion","-1;;;;");
...
if ($nombre != NULL && $email != NULL)
{
	...
	else
	{
		...
		$tabla		= "usuarios";
		$columna	= "codigo_cliente, nombre, email, tel_oficina, tel_oficina2, tel_casa, celular, direccion, barrio, clave_acceso,";
		$columna	.= " iglesia_congrega, activo";
		$valores	= "'$codigo_cliente', '$nombre', '$email', '$tel_oficina', '$tel_oficina2', '$tel_casa', '$celular', '$direccion',";
		$valores	.= " '$barrio', '$clave_acceso', '$iglesia_congrega', 'no'";
		$query = ing_datos_tabla($tabla,$columna,$valores);
		$query = act_datos_tabla("actualizacion","num_afiliado='$num'","",0);
		
		$fecha = date("j-m-Y H:i:s");
		
		$query = "INSERT INTO pedidos (";
		$query .= "id_factura, id_comprador, tipo, articulos, cantidades, precios_unitarios, precio, fecha_emision";
		$query .= ") VALUES (";
		$query .= "'$id_factura', '$codigo_cliente', 'Afiliación+Alquiler+Alquiler+Alquiler+Alquiler+', 'Afiliación a videoexpress+Pelicula de cortesia 1+Pelicula de cortesia 2+Pelicula de cortesia 3+Pelicula de cortesia 4+', '1+1+1+1+1+', '$afiliacion+0+0+0+0+', '$afiliacion', '$fecha')";
		mysql_query($query) or die(mysql_error());
	
		$asunto = "Afiliacion nueva";
		$cuerpo = "El señor $nombre se ha afiliado hoy, por favor contactarlo para activar afiliación<p>Sus telefonos son: $tel_oficina, $tel_oficina2, $tel_casa, $celular</p>";
...
<p>Recuerde que el costo de la afiliaci&oacute;n es de $ <?php echo number_format($afiliacion) ?>, y que al momento de afiliarse usted recibir&aacute; dos pel&iacute;culas completamente gratis.</p>
<p>El costo por pel&iacute;cula alquilada es de $ <?php echo number_format($alquiler) ?>.</p>
<form name="afiliacion" id="afiliacion" method='post' action='afiliacion.php' onSubmit="return comprobar_afiliacion(this)">
<table width="600" border="0" cellspacing="2" cellpadding="0" align="center" >
  ...
  <tr>
    ...
    <td><input name="clave" type="password" id="clave" tabindex="9" size="30" maxlength="150" /></td>
  </tr>
  <tr>
  	<td>*</td>
    <td>Iglesia en la que se congrega</td>
    <td><input name="iglesia" type="text" id="iglesia" tabindex="10" size="47" maxlength="500" /></td>
  </tr>
  <tr>
    <td colspan="3" align="center"><a href="#" onclick="javascript:document.afiliacion.reset()"><img src="../Imagenes_pagina/limpiar.png" width="100" height="26" border="0" /></a>&nbsp;<input type="image" src="../Imagenes_pagina/afiliarse.png" name="submit" value="submit" tabindex="11" /></td>
  </tr>
  <tr>
  	<td>*<br />**<br />***</td>
