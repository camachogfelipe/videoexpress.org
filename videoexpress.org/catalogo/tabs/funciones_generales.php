...
function recomendadas()
{
	...
	for ($i=1; $i<=sizeof($rec); $i++)
	{
		...
		//$sql = "select * FROM catalogo WHERE id='$rec[$No_items]'";
...
function mail_contacto($datos)
{
	...
	$email_envio = "felipecamachogonzalez@gmail.com";
...
{
	$asunto = "Nueva contraseña de VideoExpress.org";
	$cuerpo = "Su nueva contraseña es videoexpress, está contraseña es generica por lo que debe cambiarla al momento de ingresar nuevamente en nuestro portal<p>Administración VideoExpress.org</p>";
	$headers = "X-Mailer:PHP/".phpversion()."\n"; $headers .= "Mime-Version: 1.0\n"; $headers .= "Content-Type: text/html; charset=iso-8859-1\n";
