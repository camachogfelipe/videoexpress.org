...
if (isset($_SESSION['user_admin'])){
 ...
 $pdf->Cell(0,5,'Días trabajados capital','LTB',0,'C');
 $pdf->SetFontSize(9);
 ...
 for($a=$inicio; $a<=$year; $a++){
		$total_anio[$a]		= $anio[$a][1] + $anio[$a][6];
	}
 ...
 for($k=1;$k<$contadoranos;$k=$k+8){  
  ...
  (($inicio+8)<$year)?$pdf->Cell(0,5,'',1,1):
   ...
   $pdf->Cell($unidad,5,$item[0],1,0,'L',1);
  	for($a=$inicio; $a<=$fin; $a++)	{
  	 $pdf->Cell($unidad,5,$anio[$a][0],1,0,'L',1);
  	}
   (($inicio+8)<$year)?$pdf->Cell(0,5,'',1,1): 
   $pdf->Cell($unidad,5,$total[1],1,1,'L',1);
   
   $pdf->SetTextColor(0,0,0);
   $pdf->SetFillColor(255,255,255);
   $pdf->Cell($unidad,5,$item[1],1,0,'L',1);
  	for($a=$inicio; $a<=$fin; $a++)	{
  	 $pdf->Cell($unidad,5,'$ '.number_format($total_anio[$a],0),1,0,'L',1);
  	}
   (($inicio+8)<$year)?$pdf->Cell(0,5,'',1,1): 
   $pdf->Cell($unidad,5,'$ '.number_format($total_total,0),1,1,'L',1);
   ...
   $pdf->SetTextColor(0,0,0);
   $pdf->SetFillColor(255,255,255);
   $pdf->Cell($unidad,5,$item[7],1,0,'L',1);
  	for($a=$inicio; $a<=$fin; $a++)	{
  	 $pdf->Cell($unidad,5,'$ '.number_format($anio[$a][7],0),1,0,'L',1);
  	}
   (($inicio+8)<$year)?$pdf->Cell(0,5,'',1,1): 
   $pdf->Cell($unidad,5,'$ '.number_format($total[7],0),1,1,'L',1); 
 ...
 $pdf->Output('dias_trabajados_capital_'.date("dmY").'.pdf','I');
 exit;
}
?>