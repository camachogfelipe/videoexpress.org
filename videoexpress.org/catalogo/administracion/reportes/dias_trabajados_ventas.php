...
if (isset($_SESSION['user_admin']))
{
	...
	echo '<div><a href="pdf_dias_trabajados_ventas.php" target="_blank" title="Exportar Pdf" class="btn_pdf">Exportar Pdf</a></div>';
	
	ingresos_y_capital();
	$year = date(Y);
	
	echo "<table width=\"100%\" cellpading=\"0\" cellspacing=\"0\" border=\"0\" align=\"center\">";
	...
	$result = consulta_bd("ingresos_y_capital","*","4;;;;0/6");
	...
	for($y=2003; $y<=$year; $y++)
	{
		echo "<td>$ ".number_format($anio[$y][1],2)."</td>";
	}
	echo "<td>$ ".number_format($total[1],2)."</td>";
	echo "</tr>";
	echo "<tr bgcolor=\"#9ABEDC\" style=\"color: #fff\">";
	echo "<td>".$item[2]."</td>";
	for($y=2003; $y<=$year; $y++)
	...
	for($y=2003; $y<=$year; $y++)
	{
		echo "<td>$ ".number_format($anio[$y][5],2)."</td>";
	}
	echo "<td>$ ".number_format($total[5],2)."</td>";
	echo "</tr>";
	echo "</table>";
	echo "</table>";
	...
	echo "<td>".$fecha2[0]."-".$fecha2[1]."-".$fecha2[2]."</td>";
