...
obligatorio = ["identificacion", "nombres", "direccion", "telefono", "celular", "email", "ciudad"];

//Pon en la veriable textoObligatorio el texto que quieres que aparezca en el alert

textoObligatorio=["Identificaci&oacute;n", "Nombre completo", "Direcci&oacute;n", "Tel&eacute;fono", "Celular", "Correo electr&oacute;nico", "Ciudad"];

function comprobar(este)
{
	for(a=0; a<obligatorio.length; a++)
	{
		if (este.elements[obligatorio[5]].value != "")
		{
			isMail(este.elements[obligatorio[5]].value);
			if (x == 1)
			{
				este.elements[obligatorio[5]].focus();
				return false;
			}
		}
...
if(isset($_SESSION['codigo_cliente'])) $identificacion=$_SESSION['codigo_cliente'];
...
{
	if($existe == 1) $id_comprador = $_SESSION['codigo_cliente'];
	elseif($existe == 0) $id_comprador = $identificacion;
		
	$contador=0;
	$suma=0;
	$subto_compra = $subto_alquiler = $suma = $ta = $tc = $tmp = 0;
	$articulos = $cantidades = $precios_unitarios = $tipo = NULL;
	$precio = 0;
	...
	foreach($carro as $k => $v)
	{
	    ...
	    if ($v['tipo_pedido'] == 'alquiler')
		{
			...
			{
				$precios_unitarios .= $_SESSION['alquiler']."+";
				$precio1 = $precios_unitarios * $cantidades['cantidad'];
				$subto_alquiler += $precio1;
			}
		}
	...
	$tabla 		= "pedidos";
	$columna	= "id_comprador, tipo, articulos, cantidades, precios_unitarios, precio, fecha_emision, id_peliculas";
	$valores	= "'$id_comprador', '$tipo', '$articulos', '$cantidades', '$precios_unitarios', '$precio', '$fecha', '$id_peliculas'";
	$query		= ing_datos_tabla($tabla,$columna,$valores);
	
	if($existe == 1)
	{
		$query = act_datos_tabla("usuarios","ultimo_alquiler='$fecha'",0,0);
	}
	elseif($existe == 0)
	{
		$total = dato_columna("clientes","COUNT(*)","1;;;id_cliente='$identificacion';");
		//$sql = "select COUNT(*) FROM clientes WHERE id_cliente = '$identificacion'";
		...
		//$total = $total_resultados[0];
		
		if($total == 0)
		{
			$tabla = "clientes";
			$columna = "id_cliente, nombres, direccion, telefono, celular, email, ciudad, pais";
			$valores = "'$identificacion', '$nombres', '$direccion', '$telefono', '$celular', '$email', '$ciudad', '$pais'";
			$query = ing_datos_tabla($tabla,$columna,$valores);
		}
		else
		{
			$tabla = "clientes";
			$datos = "nombres='$nombres', direccion='$direccion', telefono='$telefono', celular='$celular', email='$email',";
			$datos .= " ciudad='$ciudad', pais='$pais'";
			$concepto = "id_cliente='$identificacion'";
	...
	$id_ingresado = $id_comprador;
	include('mail_factura.php');
	
	$mensaje = "<img src=\"../Imagenes_pagina/informacion.png\" width=\"25\" height=\"25\" align=\"absmiddle\" /> Gracias, la operaci&oacute;n se ha realizado con exito<br />Favor verificar el correo de confirmación al e-mail ingresado";
}
else
{	
	$query = consulta_bd("usuarios","*","1;;;codigo_cliente = '$identificacion';");
	...
	{
	  	$nombres		= $_SESSION['usuario_afiliado'];
...
?>
<form action="confirmar_pedido.php?continuar=true&existe=<?php echo $existe ?>" method="post" enctype="multipart/form-data" name="confirmacion_datos" id="confirmacion_datos" onSubmit="return comprobar(this)">
<table width="600" border="0" align="center" cellpadding="0" cellspacing="2px">
  <tr>
    ...
    <td width="180">Identificaci&oacute;n</td>
    <td width="400"><input name="identificacion" type="text" id="identificacion" tabindex="1" size="30" maxlength="20" value="<?php echo $identificacion ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Nombre completo</td>
    <td><input name="nombres" type="text" id="nombres" tabindex="2" size="60" maxlength="100" value="<?php echo $nombres ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Direcci&oacute;n</td>
    <td><input name="direccion" type="text" id="direccion" tabindex="4" size="60" maxlength="150" value="<?php echo $direccion ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Tel&eacute;fono</td>
    <td><input name="telefono" type="text" id="telefono" tabindex="5" size="12" maxlength="10" value="<?php echo $telefono ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Celular</td>
    <td><input name="celular" type="text" id="celular" tabindex="6" size="12" maxlength="10" value="<?php echo $celular ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Correo electr&oacute;nico</td>
    <td><input name="email" type="text" id="email" tabindex="7" size="60" maxlength="150" value="<?php echo $email ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
    <td id="carrito">Ciudad</td>
    <td><input name="ciudad" type="text" id="ciudad" tabindex="8" size="60" maxlength="150" value="<?php echo $ciudad ?>" /></td>
  </tr>
  <tr>
    <td align="center" valign="middle" id="obligatorio">*</td>
