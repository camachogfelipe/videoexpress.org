...
<?php if($news_amount > 0) : ?>
	<div class="nsp_main<?php if($this->config['autoanim'] == TRUE) echo ' autoanim'; ?><?php if($this->config['hover_anim'] == TRUE) echo ' hover'; ?>" id="nsp-<?php echo $this->config['module_id']; ?>" style="width:<?php echo $this->config['module_width']; ?>%;">
		
		<?php if(($this->config['news_column'] * $this->config['news_rows']) > 0) : ?>
			
			<div class="nsp_arts<?php echo ' '.$this->config['links_position']; ?>" style="width:<?php echo $arts_width-0.1; ?>%;">
				...
				<?php for($i = 0; $i < count($news_html_tab); $i++) : ?>
					...
					?>
					<div class="nsp_art<?php echo $class; ?>" style="width:<?php echo 100 / $this->config['news_column']; ?>%;<?php echo $style; ?>"><div style="padding:<?php echo $this->config['art_padding']; ?>"><?php echo $news_html_tab[$i];?></div></div>
				<?php endfor; ?>	
			</div>
		<?php endif; ?>
		
		<?php if($this->config['news_short_pages'] > 0 && count($news_list_tab) > 0 ) : ?>
		<div class="nsp_links_wrap<?php echo ' '.$this->config['links_position']; ?>" style="width:<?php echo $links_width; ?>%;">
			<div class="nsp_links" style="margin:<?php echo $this->config["links_margin"]; ?>;">
				...
				<?php if(
						...
						$this->config['news_short_pages'] > 1 &&
					 	floor(count($news_list_tab) / $this->config['links_amount']) >= 1 &&
				...
				<div class="nsp_bottom_interface">
					<div>
						...
						<ul class="pagination">
							<?php for($i = 0; $i < ceil(count($news_list_tab) / $this->config['links_amount']); $i++) : ?>
							<li><?php echo $i+1; ?></li>
							<?php endfor; ?>
						</ul>
						<?php endif; ?>
