...
if($this->useMoo == 2)
{
	...
	if(!$mootools_founded)
	{
		...
		$headData["scripts"][$uri->root().'templates/gk_memovie/js/mootools.js'] = "text/javascript";
...
if($this->useScript == 2)
{
	...
	// searching phrase mootools in scripts paths
	if(array_search($uri->root().'templates/gk_memovie/js/gk_image_show.js', $headData_keys) > 0)
	{
	...
	{
		// add new script tag connected with mootools from module
		$headData["scripts"][$uri->root().'templates/gk_memovie/js/gk_image_show.js'] = "text/javascript";
		// if added mootools from module then this operation have sense
		$document->setHeadData($headData);
	}
}
