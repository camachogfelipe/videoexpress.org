...
var GKIToolbox = new Class({
	...
	start: function(){
		...
		this._overlay = $('gk-overlay');
		this._mainbox = $('gk-toolbar-main');
		if (this.options.animOn) {
			this._boxes.setStyle ('opacity', 0);
			this._overlay.setStyle ('opacity', 0);
			this._boxes.push ($('gk-toolbar-main'));
	...
	togglebox: function (link) {
		...
		if (this._currenttoggle == null) {
			//show overlay
			this._overlay.setStyles ({'display':'block', 'height':$('gk-wrapper').offsetHeight});
		}
		this.showbox (link, true);
	...
	showbox1: function (link, addback) {
		...
		this._mainbox.setStyle ('height', (link._h - 60) + "px");
