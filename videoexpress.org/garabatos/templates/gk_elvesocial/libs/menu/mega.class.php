...
if (!defined ('_GK_MEGA_MENU_CLASS')) {
	define ('_GK_MEGA_MENU_CLASS', 1);
	require_once (dirname(__FILE__).DS."base.class.php");

	class GKMenuMega extends GKMenuBase{
		...
		function beginMenu($startlevel=0, $endlevel = 10){
			echo "<div class=\"gk-megamenu\">\n";
		}
		function endMenu($startlevel=0, $endlevel = 10){
			//If rtl, not allow slide and fading effect
			$rtl = $this->getParam ('rtl');
			$animation = $this->_tmpl->getParam ('gk_menu-mega-animation', 'none');
			$duration = $this->_tmpl->getParam ('gk_menu-mega-duration', 300);
			$fade = 0;
			$slide = 0;
			if (!$rtl) {
