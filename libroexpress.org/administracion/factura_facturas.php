...
if (isset($_SESSION['valid_user']))
{
   ...
   if($tabla == "facturas")
   {
	   $sql = "select dolar_factura FROM dolar_fac_libro WHERE id_factura = '$id'";
	   $result = mysql_query($sql) or die("La siguiente consulta contiene alg&uacute;n error:<br>nSQL: <b>$sql</b>");
	   while ($row = mysql_fetch_object($result))
	   {
...
<span style="font-size:9px">US$ / CO$</span></td>
        ...
        ?>
          <tr style="color:#000" bgcolor="<?php echo $color ?>">
            ...
            <td style="border-right:1px solid #66CC99">$ <?php echo $pesos = number_format($sin_iva2[$i] / $trm,2)." / $ ".number_format($sin_iva2[$i],2); ?></td>
            ...
            <td width="11%" style="border-right:1px solid #66CC99">$ <?php echo $pesos = number_format($sin_iva[$i] / $trm,2)." / $ ".number_format($sin_iva[$i],2); ?></td>
            <td>$ <?php $pesos1 = $precios_unitarios[$i] * $cantidades[$i]; echo $pesos = number_format($pesos1 / $trm,2)." / $ ".number_format($pesos1,2); ?></td>
          </tr>
          <?php
         $i++;
        }
      ...
      <tr>
        <td height="28" style="border:1px solid #66CC99"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="14%" align="center" style="border-right:1px solid #66CC99; color:#000">$ <?php echo $pesos = number_format($to_sin_iva / $trm,2)." / $ ".number_format($to_sin_iva,2); ?></td>
            <td width="8%" align="center" style="border-right:1px solid #66CC99; color:#000"><?php echo $to_cantidad ?></td>
            <td width="16%" align="center" style="border-right:1px solid #66CC99; color:#000">$ <?php echo $pesos = number_format($to_iva / $trm,2)." / $ ".number_format($to_iva,2); ?></td>
            <td width="12%" align="center" style="color:#000">$ <?php echo $pesos = number_format($to_con_iva / $trm,2)." / $ ".number_format($to_con_iva,2); ?></td>
          </tr>
          <tr>
